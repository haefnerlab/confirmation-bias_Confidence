...
hprs_to_use = ones(2,6,3);

for j=1:cases
    for cs=1:6
        temp = [1 1 1];
        for i=1:num_sub
            temp = temp.*load_all_subj.conf_analysis{i}{j}{cs}.hprs_used;
        end
        hprs_to_use(j,cs,1) = nthroot(temp(1),num_sub);
        hprs_to_use(j,cs,2) = nthroot(temp(2),num_sub);
        hprs_to_use(j,cs,3) = nthroot(temp(3),num_sub);
    end
end
...
for i=1:(num_sub)
    ...
    for j=1:cases
        ...
        [conf_analysis{i}{j},thresh(i,j,:),count_trials_thresh(i,j)] = run_confidence_regression_both_ratio_and_noise(subjects{i},phase,perf_lo,perf_hi,boots,squeeze(hprs_to_use(j,:,1)),squeeze(hprs_to_use(j,:,2)),squeeze(hprs_to_use(j,:,3)),standardize,folds,dir,thresh_trials_only);
        for cs=1:6
