function [best_hprs, log_likelihoods] = xValidatePK_with_lapse(data, responses, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds)
...
parfor ii=1:numel(log_likelihoods)
    ...
    [params, ~, ~, ~, ~, ~] = ...
        CustomRegression.PsychophysicalKernel_with_lapse(data(dataIdx, :), responses(dataIdx), ...
...
lapse = params(end)^2;%1e-4+(1-1e-4) * sigmoid(params(end));
logits = data * weights(:);
