...
params = Model.newModelParams('frames',10, 'model', 'is_spatial', 'var_x', 0.1, 'gamma', 0.1, 'noise', 0, 'trials', 10000, 'updates', 1, 'samples', 5);
...
sens_cat_pts = Model.getThresholdPoints(ps, params, THRESHOLD, 20);
...
% [samples_var_fig, ~] = Model.plotCSPKSamplesVar(ps, ps, params, [0 0 0], 'beta', beta_range, sens_cat_pts_new);
...
hold on;
plot((0:1),zeros(1,2),'-.k','LineWidth',2);
hold on;
plot(ones(1,2),linspace(-20,20,2),'-.k','LineWidth',2);
xlabel('Frames');
ylabel('LPO');
