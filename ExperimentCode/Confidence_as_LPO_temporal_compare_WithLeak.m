...
THRESHOLD = 0.7;
pts = 4;
% Sampling model with gamma = 0.05
params = Model.newModelParams('frames',10, 'model', 'is_temporal', 'var_x', 0.1, 'gamma', 0.05, 'noise', 0, 'trials', 10000, 'updates', 5, 'samples', 5);
params_ideal = Model.newModelParams('frames',10, 'model', 'ideal', 'var_x', 0.1, 'gamma', 0.05, 'noise', 0, 'trials', 10000, 'updates', 5, 'samples', 5);
%%
...
% [conf_lshc_ideal(1), conf_lshc_ideal(2), conf_lshc_ideal(3)] = meanci(abs(results_lshc_ideal.lpo(:, end)), .67);
% [conf_hslc_ideal(1), conf_hslc_ideal(2), conf_hslc_ideal(3)] = meanci(abs(results_hslc_ideal.lpo(:, end)), .67);


% model_series = [conf_lshc(1) conf_lshc_ideal(1); conf_hslc(1) conf_hslc_ideal(1)];
% model_error_low = [conf_lshc(1)-conf_lshc(2) conf_lshc_ideal(1)-conf_lshc_ideal(2); conf_hslc(1)-conf_hslc(2) conf_hslc_ideal(1)-conf_hslc_ideal(2)];
% model_error_high = [conf_lshc(3)-conf_lshc(1) conf_lshc_ideal(3)-conf_lshc_ideal(1); conf_hslc(3)-conf_hslc(1) conf_hslc_ideal(3)-conf_hslc_ideal(1)];
% 
% 
% labels = {'Model','Ideal'};
% b = bar([1 2],model_series, 'grouped','FaceColor','flat');
% hold on
% b(1).CData(1,:) = [1 0 0]; % group 1 1st bar
% b(1).CData(2,:) = [0 0 1]; % group 1 2nd bar
% b(2).CData(1,:) = [1 0 0]; % group 2 1st bar
% b(2).CData(2,:) = [0 0 1]; % group 2 2nd bar
% b(2).LineWidth = 5;
...
%     errorbar(x, model_series(:,i), model_error_low(:,i), model_error_high(:,i),'ok', 'linestyle', 'none','linewidth', 2);
% end
...
% model_series = [conf_lshc(1) conf_hslc(1); conf_lshc_ideal(1)  conf_hslc_ideal(1)];
% model_error_low = [conf_lshc(1)-conf_lshc(2) conf_hslc(1)-conf_hslc(2); conf_lshc_ideal(1)-conf_lshc_ideal(2)  conf_hslc_ideal(1)-conf_hslc_ideal(2)];
% model_error_high = [conf_lshc(3)-conf_lshc(1) conf_hslc(3)-conf_hslc(1); conf_lshc_ideal(3)-conf_lshc_ideal(1) conf_hslc_ideal(3)-conf_hslc_ideal(1)];
model_series = [conf_lshc conf_hslc; conf_lshc_ideal  conf_hslc_ideal];
model_error = [err_lshc err_hslc; err_lshc_ideal  err_hslc_ideal];


labels = {'Approximate inference model','Ideal observer'};
b = bar([1 2],model_series, 'grouped','FaceColor','flat');
hold on
b(1).CData(1,:) = [1 0 0]; % group 1 1st bar
b(1).CData(2,:) = [0.5 0 0]; % group 1 2nd bar
b(2).CData(1,:) = [0 0 1]; % group 2 1st bar
b(2).CData(2,:) = [0 0 0.5]; % group 2 2nd bar
...
text([0.55 1.75],[4 1.5],labels,'FontSize',20,'FontWeight','bold');
