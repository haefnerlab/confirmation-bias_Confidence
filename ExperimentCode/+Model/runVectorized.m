...
results = struct(...
    ...
    'samples', zeros(trials, params.samples, frames, params.updates));
...
switch lower(params.model)
    case 'is_temporal'
        updateFun = @Model.isLogOddsUpdate_temporal;
    case 'is_spatial'
        updateFun = @Model.isLogOddsUpdate_spatial;
    case 'vb'
...
if strcmp(params.model,'is_spatial')
    results.lpo = zeros(trials, 2);
    results.lpo(:, 1) = log(prior_C) - log(1 - prior_C);
    [results.lpo(:,2), results.samples] = updateFun(params, data(:, :), results.lpo(:, 1));
else
    for f=1:frames
        [results.lpo(:, f+1), temp_samples] = updateFun(params, data(:, f), results.lpo(:, f));
        results.samples(:,:,f,:) = reshape(temp_samples, trials, params.samples, 1, params.updates);
    end
end
